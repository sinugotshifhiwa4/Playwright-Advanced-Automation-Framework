...
export default class AuthStorageManager {
  ...
  public static createAuthDirectoryIfNeeded(): void {
    try {
      ...
      AsyncFileManager.ensureDirectoryExists(dirPath);
  ...
  public static async deleteAuthStateDirectory(): Promise<boolean> {
    try {
      ...
      const doesDirExist = await AsyncFileManager.doesDirectoryExist(dirPath);

      if (doesDirExist) {
        AsyncFileManager.removeDirectory(dirPath);
  ...
  public static async getAuthStateAgeInMinutes(): Promise<number> {
    try {
      ...
      if (!(await AsyncFileManager.doesFileExist(filePath))) {
        return -1;
      }
  ...
  public static initializeEmptyAuthStateFile(): boolean {
    ...
    try {
      ...
      AsyncFileManager.writeFile(filePath, JSON.stringify({}), 'authStateFile', FileEncoding.UTF8);
