...
export default class SecureKeyGenerator {
  ...
  public static generateBase64IV(length: number = this.IV_LENGTH): string {
    if (length <= 0)
      ErrorHandler.logAndThrow('IV length must be greater than zero.', 'generateBase64IV');
    try {
  ...
  public static generateBufferIV(length: number = this.IV_LENGTH): Buffer {
    if (length <= 0)
      ErrorHandler.logAndThrow('IV length must be greater than zero.', 'generateBufferIV');
    try {
  ...
  public static generateWebCryptoIV(length: number = this.WEB_CRYPTO_IV_LENGTH): Buffer {
    if (length <= 0)
      ErrorHandler.logAndThrow('IV length must be greater than zero.', 'generateWebCryptoIV');
  ...
  public static generateBase64Salt(length: number = this.SALT_LENGTH): string {
    if (length <= 0)
      ErrorHandler.logAndThrow('Salt length must be greater than zero.', 'generateBase64Salt');
    try {
      return crypto.randomBytes(length).toString(this.BASE_64);
    } catch (error) {
      ErrorHandler.captureError(
